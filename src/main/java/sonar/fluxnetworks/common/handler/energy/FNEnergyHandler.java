...
public class FNEnergyHandler implements ITileEnergyHandler, IItemEnergyHandler {
    ...
    public boolean canRenderConnection(@Nonnull TileEntity tile, Direction side) {
        return !tile.isRemoved() && tile.getCapability(FNEnergyCapability.FN_ENERGY_STORAGE, side).isPresent();
    }

    @Override
    public boolean canAddEnergy(TileEntity tile, Direction side) {
        if(canRenderConnection(tile, side)) {
            IFNEnergyStorage storage = tile.getCapability(FNEnergyCapability.FN_ENERGY_STORAGE, side).orElse(null);
    ...
    public boolean canAddEnergy(ItemStack stack) {
        IFNEnergyStorage storage = stack.getCapability(FNEnergyCapability.FN_ENERGY_STORAGE, null).orElse(null);
        if(storage != null) {
            return storage.canReceiveL();
        }
        return false;
